# Hyprland + Noctalia / Caelestia — Native Tiling 🪟🪽

Installing Hyprland (native tiling Wayland compositor — **dwindle** default, **master** optional) with a desktop shell on an existing Arch Linux install. Two shell options are offered — pick **one**:
...
>
> **Sources:** [Hyprland wiki](https://wiki.hypr.land/) (crawled 2026-08-11), [Noctalia v5 docs](https://github.com/noctalia-dev/noctalia), [Caelestia shell](https://github.com/caelestia-dots/shell), [AUR `caelestia-shell`](https://aur.archlinux.org/packages/caelestia-shell), [SDDM](https://github.com/sddm/sddm).
...
7. [Caelestia Shell Setup (secondary)](#caelestia-shell-setup-secondary)
...
| **Noctalia** *(default)* | `noctalia-git` | AUR | Desktop shell: bar, launcher, notifications, lock, idle, wallpaper, OSD, clipboard |
...
**Deliberately NOT installed:** `hyprlock`, `hypridle`, `hyprpaper`, `hyprpicker`, `hyprsunset`, `waybar`, `mako`, `rofi`, `fuzzel` — all provided by Noctalia. Also no `cliphist` / `wlsunset` / `swayidle` / `swaylock` — Noctalia v5 has those built in. (The Caelestia path uses `walker` as its launcher instead of a built-in.)
...
### 4b. Caelestia shell (ALTERNATIVE — secondary, instead of step 4)
...
*Sources: [Hyprland wiki](https://wiki.hypr.land/) (crawled 2026-08-11 — Lua config, native tiling layouts, SDDM compat, XWayland), [Noctalia v5](https://github.com/noctalia-dev/noctalia), [Caelestia shell](https://github.com/caelestia-dots/shell), [AUR `caelestia-shell`](https://aur.archlinux.org/packages/caelestia-shell), [SDDM bug #1476](https://github.com/sddm/sddm/issues/1476).*
