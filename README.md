...
| [KDE Plasma](KDE_Plasma.md) 🖥️ | Companion — KDE install (PAM auto-provided since 6.7), apps, KWallet (+ [theming](KDE_Theming.md)) |
...
- **Niri guide — Keybind Additions missing `Mod+Shift+S`:** the copy-paste block in `Niri_Noctalia_v5.md` omitted the rectangle-screenshot bind (present in the full config sample at line 450 and in Quick Reference, but absent from the DE Applications block users actually copy). Added `Mod+Shift+S` — grim/slurp/satty pipeline (see the grim/slurp/satty bullet above; the earlier `spectacle -r` version was superseded the same day when Spectacle proved non-functional on Niri).
- **Niri guide — Satty floating window rule:** new **Satty Floating Window** subsection under KDE file picker/dialog floating rules — `window-rule { match app-id="com.gabm.satty"; open-floating true; ... }` (app ID is `com.gabm.satty`, not `satty`). Verified on pop_arch 2026-08-28: satty opens floating 934×600 centered. Live config updated + reloaded. Also added the rule to the full config sample (after the Noctalia floating rule).
- **Niri guide — Kitty CJK/Thai tofu fix:** new **Kitty CJK / Thai Tofu (Missing Glyphs)** troubleshooting subsection — multiple `font_family` lines are last-wins (not a fallback chain); use `symbol_map` per Unicode range (Thai U+0E00-U+0E7F, CJK ideographs, Kana/fullwidth). Verified on pop_arch 2026-08-28 via `kitty --debug-font-fallback`.
...
- **Niri guide — desktop monitor brightness via DDC/CI + centering overrides + Brave/kitty troubleshooting:** `Niri_Noctalia_v5.md` — keybind example now binds `XF86MonBrightness*` to a `~/.local/bin/brightness.sh` DDC/CI wrapper instead of `noctalia msg brightness-*` (which only controls the sysfs backlight on **laptops** — desktop monitors silently ignore it). New **Desktop Monitor Brightness (DDC/CI)** subsection: `ddcutil` + i2c udev rule (0660), wrapper script, and the MSI MAG341CQ note that DDC/CI writes are rejected until enabled in the monitor OSD (`DDCRC_VERIFY` — reads work, writes fail). New **Centering Behavior** subsection: `center-focused-column` (`never`/`always`/`on-overflow`) + `always-center-single-column` via a `user-override.kdl` included **after** `noctalia.kdl` (later include wins the merge; verified on pop_arch 2026-08-24, user picked `on-overflow` + `always-center-single-column`). KDE Integration TWO-places note extended: **IPC-spawned processes don't inherit niri's `environment {}` block mid-session** — re-login or `systemctl --user set-environment`. Troubleshooting gains **Brave "Set as Default Browser" won't persist** (diagnosis: `First Run` marker lingering, `xdg-settings get default-web-browser`, remove marker) and **Kitty opacity looks solid** (check `background_opacity`/`dynamic_background_opacity`/`toggle_opacity` + what's behind the terminal; match another machine by copying theme files byte-for-byte — md5 identical on pop_arch vs Chuwi).
...
- **Niri guide — Plasma/KWin no longer required (path-aware cleanup):** `Niri_Noctalia_v5.md` — intro no longer assumes an "existing KDE Plasma" install; package list, KDE Integration (Required Services), env blocks/tables, Plasma Integration (Optional), Migration Notes, and troubleshooting rows now mark `plasma-integration` / `polkit-kde-agent` / `xdg-desktop-portal-kde` / `plasma-workspace` as **Noctalia-path-only**, with the DMS path using `qt6ct-kde` + `polkit-gnome` + `xdg-desktop-portal-wlr` (verified on pop_arch — 19 Plasma/KWin packages removed, Dolphin/Kate still themed). Migration Notes updated: keep only `kded`, both `plasma-workspace` + `plasma-integration` removable on DMS path. "Session File for Plasma Login Manager" renamed → **Session File for SDDM** (TOC + links). Fixed 2 pre-existing broken anchors (`#cursor-theme`, `#default-applications`).
...
- **Main guide gains automatic time sync + closes 3 install gaps:** `Arch Linux_Installation_Guide.md` now enables `systemd-timesyncd` (NTP) in §6.2 (was missing entirely — clock would drift after every reboot), fixes the AMD GPU path in §8.3 (detection set `gpu_vendor="amd"` but had no install block — now `mesa vulkan-radeon radeontop` + tools), makes mDNS actually work (§6.1/§6.2: `avahi nss-mdns` was installed but never wired into `/etc/nsswitch.conf` nor had `avahi-daemon` enabled — added the `mdns_minimal [NOTFOUND=return]` hosts line + service), and adds **`paccache.timer`** (auto pacman-cache cleanup via `pacman-contrib`, §6.1/§6.2 + §8.11 manual fallback). Bonus: added `fwupd` + `fwupd-refresh.timer` (firmware updates) and `power-profiles-daemon` (KDE power profiles, `powerprofilesctl`), and fixed a pre-existing **unclosed code fence at §8.8** that was swallowing §8.9+ as one giant code block.
